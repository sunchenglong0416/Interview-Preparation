...
* 底层硬件指令实现
写操作：
  * 当一个线程对 volatile 变量进行写操作时，会在写操作之后插入一个 StoreStore 屏障 和一个 StoreLoad 屏障。
  * StoreStore 屏障能够确保在 volatile 写操作之前的所有普通写操作都已经刷新到主内存；StoreLoad 屏障则保证 volatile 写操作之后的读操作不会重排序到写操作之前。
读操作：
  * 当一个线程对 volatile 变量进行读操作时，会在读操作之前插入一个 LoadLoad 屏障 和一个 LoadStore 屏障。
  * LoadLoad 屏障保证 volatile 读操作之后的普通读操作不会重排序到读操作之前；LoadStore 屏障确保 volatile 读操作之后的写操作不会重排序到读操作之前。
